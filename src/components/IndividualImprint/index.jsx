...
import { Accordion, Text, Box, Svg, pulse } from 'components';
...
export default function IndividualImprint({ userInfo }) {
  ...
  const regularTabs = [
    {
      settingsName: t('Height & Weight & BMI Information'),
      body: (
        <SomeInfoContainer>
            ...
            userInfo?.vki ? (
              ...
              >
                <span style={{ margin: 10 }}>
                  {t('age')}:{' '}
                  <span style={{ color: 'var(--blue)' }}>{userInfo?.age}</span>
                </span>
                <br />
                <span style={{ margin: 10 }}>
                  {t('height')}:{' '}
                ...
                <span style={{ margin: 10 }}>
                  {t('BMI')}:{' '}
                  <span style={{ color: 'var(--blue)' }}>{userInfo?.vki}</span>
                </span>
              </div>
            ) : (
              <span
                style={{ width: '100%', color: 'var(--blue)', fontWeight: 500 }}
              >
                {t('Height and weight information cannot be displayed')}
              </span>
            )}
          </div>
        </SomeInfoContainer>
      ...
      body: (
        <PARQContainer>
          {userInfo?.parq?.length > 0 ? (
            ...
            >
              {t('This user has not completed the PARQ test')}
            </span>
          )}
        </PARQContainer>
      ),
          ...
          {userInfo?.appointments?.counts?.branches?.length > 0 ? (
            <>
              <div className="static-heads">
                <span className="head">{t('branch')}</span>
                <span className="head">{t('Number of Courses Completed')}</span>
              </div>
            ...
            >
              {t('This user has not completed any lessons on the 321 system')}{' '}
            </span>
          )}
        </Appointments>
      ),
